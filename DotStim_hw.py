...
exp_info = {'participant_nr': '', 'age': '', 'difficulty': ['easy', 'hard'], 'number of trials':[10,50,100,150]}
dlg = DlgFromDict(exp_info)
trialn= exp_info['number of trials']

if exp_info['difficulty'] == 'easy':
    COHERENCE_LEVELS = [0.4, 0.6, 0.8]
else:
    COHERENCE_LEVELS = [0.05, 0.1, 0.2]
...
while True:
    ...
    if 'return' in keys:
        p_name=p_name
        break  # break out of the loop! #can add something to change this to prenvent people from randomly clicking
...
for trial in trials:
    ...
    if response == trial['correct_response']:
        msg, color = "Correct!", "green"
    else:
        msg, color = "Wrong!", "red"
    TextStim(win, msg, color=color, height=0.1).draw()
    win.flip()
    wait(1)
